...
func (e *StrategyEngine) BuildSystemPrompt(accountEquity float64, variant string) string {
	...
	sb.WriteString("  - ❌ WRONG: `108.28K` or `215.63M` (JSON does not support suffixes)\n")
	sb.WriteString("  - ✅ CORRECT: `108280` (if 108.28K) or `215630000` (if 215.63M)\n")
	sb.WriteString("  - **Calculate the full value: K = ×1000, M = ×1000000, then output the number**\n")
	...
	sb.WriteString("  - **MUST be a pure numeric value WITHOUT suffixes (K, M, etc.)**\n")
	sb.WriteString("  - ❌ WRONG: `32.28K` or `84.21M` (JSON does not support suffixes)\n")
	sb.WriteString("  - ✅ CORRECT: `32280` (if 32.28K) or `84210000` (if 84.21M)\n\n")
	...
	sb.WriteString("   - **JSON standard**: Only pure numbers are valid (e.g., `108280`, NOT `108.28K`)\n")
	sb.WriteString("   - **Conversion rules**: K = ×1000, M = ×1000000, B = ×1000000000\n")
	sb.WriteString("   - ❌ WRONG: `\"position_size_usd\": 108.28K` (invalid JSON syntax - will cause parsing error)\n")
	sb.WriteString("   - ✅ CORRECT: `\"position_size_usd\": 108280` (valid JSON number)\n")
	...
	sb.WriteString("- ❌ Using suffixes (K, M, B, etc.) in numeric fields - JSON parser will REJECT this!\n")
	sb.WriteString("  - Example error: `invalid character 'K' after object key:value pair`\n")
	sb.WriteString("  - Solution: Convert 108.28K to 108280, 215.63M to 215630000 before outputting\n")
